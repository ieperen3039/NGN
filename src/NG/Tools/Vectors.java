...
public final class Vectors {
    ...
    public static boolean almostEqual(Vector3fc alpha, Vector3fc beta) {
        return Math.abs(alpha.x() - beta.x()) < VECTOR_EQUALITY_DIST 
            && Math.abs(alpha.y() - beta.y()) < VECTOR_EQUALITY_DIST 
            && Math.abs(alpha.z() - beta.z()) < VECTOR_EQUALITY_DIST;
    }
    ...
    public static Vector3f toFloat(Vector3dc vec) {
        return new Vector3f(
                (float) vec.x(),
                (float) vec.y(),
                (float) vec.z());
