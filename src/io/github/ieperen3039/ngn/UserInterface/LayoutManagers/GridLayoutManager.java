package io.github.ieperen3039.ngn.UserInterface.LayoutManagers;
...
public class GridLayoutManager implements SLayoutManager {
    ...
    static int[] calculateDimensionSizes(int[] minSizes, int numGrows, boolean[] wantGrows, int size) {
        ...
        int numElements = minSizes.length;
        ...
        if (spareSize <= 0) {
            // elements don't fit, return minimum sizes
            return minSizes.clone();
        }

        int trueNumGrows = (numGrows == 0) ? numElements : numGrows;

        int growValue = spareSize / trueNumGrows;
        int roundingErrors = spareSize % trueNumGrows;

        int[] widths = new int[numElements];
        for (int i = 0; i < numElements; i++) {
            int eltWidth = minSizes[i];
            if (wantGrows[i] || numGrows == 0) {
                eltWidth += growValue;
                if (roundingErrors > 0) {
                    eltWidth += 1;
                    roundingErrors--;
        ...
        assert Arrays.stream(widths).sum() == size : "Sum of widths is not equal to size";
